...
from utils.models import NotifiedIssue, SeverityChange
...
def _mock_subprocess_ok(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("utils.teams.subprocess.run", fake_run)
    monkeypatch.setattr("utils.teams.os.path.exists", lambda _: True)
    return calls
...
def test_notify_teams_skips_when_script_not_found(
    ...
    calls: list = []
    monkeypatch.setattr("utils.teams.subprocess.run", lambda cmd, **kw: calls.append(cmd))
    monkeypatch.setattr("utils.teams.os.path.exists", lambda _: False)
