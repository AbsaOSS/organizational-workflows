...
- No label setup required: the pipeline creates any missing labels on first run
  (`scope:security`, `type:aquasec`, `epic`, `sec:suppression`, `sec:false-positive`)
...
- Required labels are created automatically if missing; the token needs `issues: write`
