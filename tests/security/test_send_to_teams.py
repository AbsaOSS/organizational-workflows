...
def test_no_body_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("send_to_teams.sys.stdin", fake_stdin)
...
def test_main_sends_when_not_dry_run(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    calls: list[tuple] = []
    monkeypatch.setattr("send_to_teams.send_to_teams", lambda url, payload: calls.append((url, payload)))
...
def test_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("send_to_teams.requests.post", fake_post)
