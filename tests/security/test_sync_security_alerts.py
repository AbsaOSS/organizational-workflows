...
"""Unit tests for ``sync_security_alerts.py``."""

import pytest
from pytest_mock import MockerFixture

from sync_security_alerts import VALID_STATES, _resolve_repo, main, parse_args
...
def _run_promote(mocker: MockerFixture, tmp_path, extra_args: list[str] | None = None) -> list[str]:
    ...
    mocker.patch("sync_security_alerts.check_labels", return_value=[])
    mocker.patch("sync_security_alerts.collect_alert_main")
    mock_promote = mocker.patch("sync_security_alerts.promote_alerts_main")
...
def test_missing_labels_returns_1(mocker: MockerFixture) -> None:
    mocker.patch("sync_security_alerts.check_labels", return_value=["epic"])
    rc = main(["--repo", REPO])
    assert rc == 1


def test_skip_label_check(mocker: MockerFixture, tmp_path) -> None:
    mock_check = mocker.patch("sync_security_alerts.check_labels")
    mocker.patch("sync_security_alerts.collect_alert_main")
    mocker.patch("sync_security_alerts.promote_alerts_main")
    out = str(tmp_path / "alerts.json")
...
def test_collect_called_with_basic_args(mocker: MockerFixture, tmp_path) -> None:
    mocker.patch("sync_security_alerts.check_labels", return_value=[])
    mock_collect = mocker.patch("sync_security_alerts.collect_alert_main")
    mocker.patch("sync_security_alerts.promote_alerts_main")
    out = str(tmp_path / "alerts.json")
...
def test_collect_error_propagates(mocker: MockerFixture, tmp_path) -> None:
    mocker.patch("sync_security_alerts.check_labels", return_value=[])
    mocker.patch("sync_security_alerts.collect_alert_main", side_effect=SystemExit(1))
    out = str(tmp_path / "alerts.json")
    with pytest.raises(SystemExit):
...
def test_promote_error_propagates(mocker: MockerFixture, tmp_path) -> None:
    mocker.patch("sync_security_alerts.check_labels", return_value=[])
    mocker.patch("sync_security_alerts.collect_alert_main")
    mocker.patch("sync_security_alerts.promote_alerts_main", side_effect=SystemExit(1))
    out = str(tmp_path / "alerts.json")
    with pytest.raises(SystemExit):
        main(["--repo", REPO, "--out", out])
...
def test_pipeline_call_order(mocker: MockerFixture, tmp_path) -> None:
    call_order: list[str] = []
    mocker.patch(
        "sync_security_alerts.check_labels",
        return_value=[],
        side_effect=lambda *a, **k: (call_order.append("check"), [])[-1],
    )
    mocker.patch(
        "sync_security_alerts.collect_alert_main",
        side_effect=lambda *a, **k: call_order.append("collect"),
    )
    mocker.patch(
        "sync_security_alerts.promote_alerts_main",
